...
graph LR
    subgraph "DANN 系統架構"
        X1["Source Image (有標籤)"] --> FE["Feature Extractor <br> (參數: &theta;_f)"]
        X2["Target Image (無標籤)"] --> FE
        FE --> FV["Feature Vector"]
        FV --> LP["Label Predictor <br> (參數: &theta;_p)"]
        FV --> DC["Domain Classifier <br> (參數: &theta;_d)"]
...
```mermaid
graph TD
    subgraph "Label Space Mismatch 分類"
        A["Closed Set DA <br> (Source 與 Target 類別完全一致)"] 
        B["Partial DA <br> (Target 類別 ⊂ Source 類別)"]
        C["Open Set DA <br> (Source 與 Target 有交集，但也各有獨特類別)"]
        D["Universal DA <br> (不對類別關係做任何假設)"]
    end
```
...
```mermaid
graph LR
    subgraph "Domain Generalization 訓練"
        S1["Domain 1: 真實貓狗"] --> Train["多網域聯合訓練 <br> (學習提取共用特徵)"]
        S2["Domain 2: 素描貓狗"] --> Train
        S3["Domain 3: 水彩貓狗"] --> Train
    end
    Train --> Test["測試於未知的 Target Domain <br> (例如: 卡通貓狗)"]
```
