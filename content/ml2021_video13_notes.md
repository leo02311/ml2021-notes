...
graph TD
    ...
    Q_Vec -- Node1["點積計算分數"] --> Attn_Scores["注意力分數"]
    K_Vec -- Node2["提供上下文"] --> Attn_Scores
    Attn_Scores -- Node3["Softmax正規化"] --> Alpha_Prime["注意力權重 α'"]
    V_Vec -- Node4["加權求和"] --> Output_B["輸出向量 b"]
    Alpha_Prime -- Node4["加權求和"] --> Output_B

    SA_Matrix --> Input_I["輸入 I 矩陣"]
    Input_I -- Node5["乘以 Wq, Wk, Wv"] --> QKV_Matrix["Q, K, V 矩陣"]
    QKV_Matrix -- Node6["Q K轉置計算"] --> Raw_Attn_A["原始注意力矩陣 A"]
    Raw_Attn_A -- Node3["Softmax正規化"] --> Attn_Matrix_A_Prime["正規化注意力矩陣 A'"]
    Attn_Matrix_A_Prime -- Node7["乘以 V 矩陣"] --> Output_O["輸出 O 矩陣"]
    QKV_Matrix -- Node8["Wq, Wk, Wv 可學習參數"] --> SA_Matrix

    MHSA --> Diff_Heads["多個獨立的頭"]
    MHSA --> Concat_Output["拼接各頭輸出"]
    MHSA --> Linear_Transform["線性轉換 W^O"]
    Diff_Heads -- Node9["捕捉不同相關性"] --> Attn_Scores
    ...
    Truncated_Attn -- Node10["降低 O("]L^2")" --> App
