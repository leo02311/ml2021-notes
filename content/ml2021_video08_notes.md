---
title: "第08堂課：【機器學習2021】類神經網路訓練不起來怎麼辦 (四)：損失函數 (Loss) 也可能有影響"
tags:
  - MachineLearning
  - ML2021
  - Classification
...
# 【機器學習 2021】第08堂課：【機器學習2021】類神經網路訓練不起來怎麼辦 (四)：損失函數 (Loss) 也可能有影響
...
*   **迴歸定義**：輸入向量，輸出一個數值 $y$，目標是讓 $y$ 接近真實標籤 $\hat{y}$。
*   **分類的類比**：
    *   可以嘗試將分類問題視為迴歸：輸入一個東西，輸出一個數值 $y$。
    *   將類別數值化：例如 Class1 編號為 1，Class2 編號為 2，Class3 編號為 3。
    *   目標：讓 $y$ 接近類別的編號。
*   **問題點**：
    *   這種數值化方式隱含了類別之間的相似性。例如，$\text{Class1}=1, \text{Class2}=2, \text{Class3}=3$，會預設 Class1 與 Class2 較接近，Class1 與 Class3 較遠。
...
當目標 $\hat{y}$ 是 One-hot Vector (一個多維向量) 時，神經網路的輸出也應該是一個多維向量。

*   **設計**：
    *   如果 $\hat{y}$ 是一個 $K$ 維向量 ($K$ 個類別)，則網路的輸出 $y$ 也應為 $K$ 個數值 ($y_1, y_2, \dots, y_K$)。
    *   實現方式：將原本輸出單一數值的網路結構重複 $K$ 次，每次使用不同的權重與偏置，即可產生 $K$ 個輸出值。

### Soft-max 函式

在分類問題中，神經網路的原始輸出 $y$ 通常會再通過一個 Soft-max 函式，得到 $y'$，然後再計算 $y'$ 與 $\hat{y}$ 之間的距離。

#### 為何需要 Soft-max？

*   **簡化解釋**：$\hat{y}$ (One-hot Vector) 的值只有 0 和 1。為了讓網路輸出 $y$ 能與 $\hat{y}$ 更好地比較相似度，我們需要將 $y$ 的值也歸一化到 0 到 1 之間。Soft-max 函式就扮演了這個角色。
*   **深層解釋 (課程中簡略)**：Soft-max 具有豐富的歷史淵源，通常從生成模型、Logistic Regression 等角度深入解釋其背後的假設。若要了解，可參考相關課程錄影。

#### Soft-max 運作原理

Soft-max 函式將輸入的 $y_1, y_2, y_3$ (Logits) 轉換為 $y_1', y_2', y_3'$。

1.  **取指數 (Exponential)**：對每個輸入 $y_i$ 取指數運算：$\exp(y_i)$。
    *   無論 $y_i$ 是正數還是負數，$\exp(y_i)$ 都會是正數。
2.  **歸一化 (Normalization)**：將每個 $\exp(y_i)$ 除以所有 $\exp(y_i)$ 的總和。
    *   $$ y'_i = \frac{\exp(y_i)}{\sum_j \exp(y_j)} $$

**圖示化說明：**  
$y_1, y_2, y_3 \to \exp(y_1), \exp(y_2), \exp(y_3) \to \text{Summation} = \exp(y_1) + \exp(y_2) + \exp(y_3) \to$
$$ y'_1 = \frac{\exp(y_1)}{\text{Summation}} $$
$$ y'_2 = \frac{\exp(y_2)}{\text{Summation}} $$
$$ y'_3 = \frac{\exp(y_3)}{\text{Summation}} $$

#### Soft-max 的特性

*   **輸出範圍**：所有 $y'_i$ 都介於 0 到 1 之間。
*   **總和為 1**：所有 $y'_i$ 的總和為 1 (類似機率分佈)。
*   **拉大數值差距**：Soft-max 會讓輸入中較大的值變得更大 (接近 1)，較小的值變得更小 (接近 0)，從而放大不同類別之間的區別。
    *   **範例**：輸入 $[3, 1, -3]$ 經過 Soft-max 後變成 $[0.88, 0.12, 0.00]$，可以看到 $-3$ 被壓到趨近於 0。

#### Logit

Soft-max 函式的輸入 $y_i$ 通常被稱為 **Logit**。
...
在得到 $y'$ (Soft-max 輸出) 和 $\hat{y}$ (One-hot label) 後，我們需要計算兩者之間的「距離」$e$，這個距離就是損失 (Loss)。
...
*   **公式**：$$ e = -\sum_i \hat{y}_i \log(y'_i) $$
    *   **註**：通常 Cross-entropy 的標準公式為 $-\sum_i \hat{y}_i \log(y'_i)$。教授的講法是當 $\hat{y}$ 與 $y'$ 相同時，該值越小越好，這與帶負號的公式在最小化目標上是一致的。
