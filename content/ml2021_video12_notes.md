...
*   **詞彙向量化**：
    *   **One-Hot Encoding**：簡單直接，但缺乏語義資訊，假設詞彙間無關聯。
        *   範例：Apple $H: (1,0,0,\dots)$, Bag $C: (0,1,0,\dots)$, Cat $(0,0,1,\dots)$。
...
*   **表示方式**：圖中的每個節點（如人、原子）可視為一個向量。
    ...
    *   **分子**：每個節點代表一個原子。
        *   原子向量化：可用 One-Hot Vector 表示（如氫 $H: (1,0,0,\dots)$, 碳 $C: (0,1,0,\dots)$）。
*   **總結**：一個圖形可表示為一堆向量。
...
Self-Attention 接收一排輸入向量 $a_1, a_2, a_3, a_4$，輸出相同數量且已考慮整個序列上下文的向量 $b_1, b_2, b_3, b_4$。以下將詳細說明如何產生 $b_1$。

#### 步驟一：計算關聯性 (Attention Score - $\alpha$)

此步驟旨在根據 $a_1$，找出序列中與 $a_1$ 相關程度高的其他向量。關聯程度用數值 $\alpha$ 表示。

1.  **Query ($q$), Key ($k$), Value ($v$) 概念**：
    *   **Query ($q$)**：由輸入向量 $a$ 乘以矩陣 $W_q$ 得到，代表「查詢」的內容，像搜尋文章的關鍵字。
    *   **Key ($k$)**：由輸入向量 $a$ 乘以矩陣 $W_k$ 得到，代表「被查詢」的內容，像文章的索引。
    *   **Value ($v$)**：由輸入向量 $a$ 乘以矩陣 $W_v$ 得到，代表「實際的資訊內容」，像文章的內容。

2.  **Dot Product 注意力機制**：
    *   這是最常見的計算 $\alpha$ 的方法，也是 Transformer 中使用的方法。
    *   **過程**：
        1.  將輸入向量 $a_i$ 乘以 $W_q$ 得到 $q_i$。
        2.  將輸入向量 $a_j$ 乘以 $W_k$ 得到 $k_j$。
        3.  計算 $q_i$ 與 $k_j$ 的**內積 (Dot Product)**，得到一個純量值 $\alpha_{i,j}$。
        *   $$ \alpha_{i,j} = q_i \cdot k_j $$

3.  **計算 $a_1$ 與其他向量的關聯性**：
    *   將 $a_1$ 乘以 $W_q$，得到 **$q_1$ (Query)**。
    *   將 $a_2, a_3, a_4$ 分別乘以 $W_k$，得到 **$k_2, k_3, k_4$ (Keys)**。
    *   計算 $q_1$ 與每個 $k_i$ 的內積，得到關聯性分數 (Attention Score)：
        *   $$ \alpha_{1,2} = q_1 \cdot k_2 $$ （$a_1$ 與 $a_2$ 的關聯性）
        *   $$ \alpha_{1,3} = q_1 \cdot k_3 $$ （$a_1$ 與 $a_3$ 的關聯性）
        *   $$ \alpha_{1,4} = q_1 \cdot k_4 $$ （$a_1$ 與 $a_4$ 的關聯性）
    *   **注意**：在實際實作中，$a_1$ 也會和自己計算關聯性：$\alpha_{1,1} = q_1 \cdot k_1$。

#### 步驟二：SoftMax 歸一化 (Normalization)

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="../assets/vid12_attn_dark.png">
  <img alt="Self-Attention Scores Heatmap" src="../assets/vid12_attn_light.png">
</picture>

*   將所有計算出的 $\alpha$ 值（如 $\alpha_{1,1}, \alpha_{1,2}, \alpha_{1,3}, \alpha_{1,4}$）通過 **SoftMax 函數**進行歸一化。
*   SoftMax 的輸出是一排經過歸一化的注意力權重 $\alpha'$（如 $\alpha'_{1,1}, \alpha'_{1,2}, \alpha'_{1,3}, \alpha'_{1,4}$）。
*   **注意**：此處不一定要使用 SoftMax，也可以嘗試其他激活函數（如 ReLU），效果可能更好。

#### 步驟三：加權求和 (Weighted Sum) 獲取上下文資訊

*   此步驟根據歸一化後的注意力權重 $\alpha'$，從序列中抽取與 $a_1$ 相關的重要資訊，生成輸出向量 $b_1$。
*   **過程**：
    1.  將輸入向量 $a_1, a_2, a_3, a_4$ 分別乘以 $W_v$，得到 **$v_1, v_2, v_3, v_4$ (Values)**。
    2.  將每個 $v_i$ 乘以對應的歸一化注意力權重 $\alpha'_{1,i}$。
    3.  將所有加權後的 $v_i$ 求和，得到最終的輸出向量 $b_1$。
        *   $$ b_1 = \alpha'_{1,1} \cdot v_1 + \alpha'_{1,2} \cdot v_2 + \alpha'_{1,3} \cdot v_3 + \alpha'_{1,4} \cdot v_4 $$
*   **意義**：$b_1$ 是序列中所有 `Value` 向量的加權平均。哪個 $\alpha'$ 值越高，其對應的 $v$ 向量對 $b_1$ 的影響就越大，代表該部分資訊與 $a_1$ 最相關。

```mermaid
graph TD
    subgraph Node1["Self-Attention 運作機制"]
    ...
    subgraph Node3["輸入類型"]
        R["變長序列輸入Node4["] --> S["]文字處理 (Word Embeddings)"]
        R --> T["聲音訊號 (Frames)"]
        R --> U["圖形資料 (Nodes as Vectors)"]
    end

    subgraph Node5["輸出類型"]
        V["模型輸出Node6["] --> W["]類型一：序列標註"]
        V --> X["類型二：整段序列單一標註"]
        V --> Y["類型三：序列到序列"]
    end

    M -- Node7["解決"] --> U_FC["傳統FC網路限制"]
    M -- Node7["解決"] --> U_Win["擴展視窗限制"]
...
<details>
  <summary>點擊展開解答</summary>
  *   **Query ($q$)**：代表「查詢」的內容，由當前輸入向量 $a_i$ 經過線性轉換 (乘以 $W_q$ 矩陣) 而來。它用於向序列中的其他元素發出「查詢」。
  *   **Key ($k$)**：代表「被查詢」的內容或「索引」，由序列中每個輸入向量 $a_j$ 經過線性轉換 (乘以 $W_k$ 矩陣) 而來。它與 Query 進行匹配，以判斷關聯性。
  *   **Value ($v$)**：代表實際的「資訊內容」，由序列中每個輸入向量 $a_j$ 經過線性轉換 (乘以 $W_v$ 矩陣) 而來。在計算出注意力權重後，這些 Value 會被加權求和，形成最終的輸出。
</details>
