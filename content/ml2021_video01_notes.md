...
```mermaid
graph TD
    ML["Machine Learning<br>讓機器具備找函式的能力"] --> Tasks["任務類別"]
    ML --> Steps["找函式的三個步驟"]

    Tasks --> REG["Regression 迴歸<br>輸出為一個數值"]
    Tasks --> CLS["Classification 分類<br>做選擇題"]
    Tasks --> STR["Structured Learning<br>產生有結構的物件"]

    Steps --> S1["Step 1: 定義 Model<br>帶有未知參數的函式"]
    Steps --> S2["Step 2: 定義 Loss<br>評估參數的好壞"]
    Steps --> S3["Step 3: 最佳化 Optimization<br>找出讓 Loss 最小的參數"]

    S1 --> Param["未知參數"]
    Param --> W["Weight 權重"]
    Param --> B["Bias 偏差值"]
    S1 --> Feat["Feature 特徵<br>已知的輸入資料"]
